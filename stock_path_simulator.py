...
if ticker:
    ...
    # Add simulation paths (bottom layer)
    colors = px.colors.qualitative.Plotly
    for i in range(min(n_sim, 50)):  # limit to 50 lines for performance
        fig.add_trace(go.Scatter(
            ...
            name=f"Sim {i+1}",
            opacity=0.3,
            showlegend=False  # Hide legends for simulations to declutter
        ))

    # Add percentiles (middle layer)
